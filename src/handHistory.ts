...
export function appendCompletedHand(history: HandRecord[], state: HandState): HandRecord[] {
  ...
  const record: HandRecord = {
    ...
    pots: state.winnerIds.map((winnerId, index) => ({
      label: state.winnerIds.length > 1 ? `Split pot ${index + 1}` : 'Main pot',
      amount: Math.floor(state.potAwarded / Math.max(1, state.winnerIds.length)),
      winnerId,
      winnerHandDescription: rankHandDescription(showdown),
